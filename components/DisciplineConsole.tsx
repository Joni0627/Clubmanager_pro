...
const DisciplineConsole: React.FC<DisciplineConsoleProps> = ({ discipline, clubConfig, members, onBack }) => {
  ...
  // Filtrar miembros que tienen el rol PLAYER asignado a esta disciplina
  const disciplineAthletes = useMemo(() => {
    return members.filter(m => 
      m.assignments.some(a => a.disciplineId === discipline.id && a.role === 'PLAYER')
    );
  }, [members, discipline.id]);

  const filteredAthletes = useMemo(() => {
    let base = disciplineAthletes;
    if (selectedCategoryId !== 'all') {
      base = base.filter(m => 
        m.assignments.some(a => a.disciplineId === discipline.id && a.categoryId === selectedCategoryId)
      );
    }
    return base;
  }, [disciplineAthletes, selectedCategoryId, discipline.id]);
        ...
        <div className="max-w-7xl mx-auto py-12 px-6 md:px-12">
            ...
            <div className="space-y-12 animate-fade-in pb-20">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="bg-white dark:bg-[#0f1219] p-10 rounded-[3.5rem] shadow-sm border border-slate-200 hover:shadow-2xl transition-all group relative overflow-hidden">
                     ...
                     <div className="flex items-end gap-5">
                        <span className="text-7xl font-black dark:text-white leading-none tracking-tighter">{disciplineAthletes.length}</span>
          ...
          {activeSubTab === 'players' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 animate-fade-in pb-20">
              {filteredAthletes.map(athlete => (
                <div 
                  key={athlete.id} 
                  onClick={() => setSelectedMember(athlete)}
                ...
                >
                  <div className="flex flex-col items-center relative z-10">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <span className="text-slate-300 font-black italic">ID: {athlete.dni.slice(-4)}</span>
                    </div>
          ...
          {activeSubTab === 'attendance' && (
            <div className="bg-white dark:bg-[#0f1219] rounded-[4rem] border border-slate-200 shadow-2xl p-6 md:p-12 animate-fade-in pb-20 overflow-hidden">
               {/* Adaptado para manejar miembros en lugar de players */}
               <AttendanceTracker players={filteredAthletes as any} clubConfig={clubConfig} forceSelectedDisc={discipline.name} />
            </div>
          )}

          {activeSubTab === 'medical' && (
            <div className="bg-white dark:bg-[#0f1219] rounded-[4rem] border border-slate-200 shadow-2xl p-6 md:p-12 animate-fade-in pb-20 overflow-hidden">
               <MedicalDashboard players={filteredAthletes as any} />
            </div>
          )}
        </div>
      </div>
