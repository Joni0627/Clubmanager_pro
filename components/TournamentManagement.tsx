...
import { Tournament, Match, MatchEvent, Player, Category, Discipline, MatchEventType } from '../types';
import { 
...
const TournamentManagement: React.FC<TournamentManagementProps> = ({ discipline, category, gender, players }) => {
  ...
  const [matchForm, setMatchForm] = useState<any>({
    homeTeam: '', awayTeam: '', date: new Date().toISOString().split('T')[0],
    status: 'Scheduled', homeScore: 0, awayScore: 0, group: 'A', stage: 'Fase Regular',
    incidents: [] // Array de { playerId, type, minute }
  });
  ...
  const handleSaveMatch = async () => {
    if (!activeTournament || !matchForm.homeTeam || !matchForm.awayTeam) return;
    try {
      const m = {
  ...
  // Fix: Added deleteMatch function to handle match deletion from Supabase
  ...
  // Cálculo de Tabla de Posiciones simplificado (UI Only)
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        {/* Sidebar: Tournament Selector & Local Stats */}
                  ...
                  <>
                    <div className="bg-white dark:bg-[#0f1219] p-10 rounded-[4rem] border border-slate-200 dark:border-white/5 shadow-xl relative overflow-hidden">
                      <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
                        ...
                        <button 
                          onClick={() => { setMatchForm({ homeTeam: activeTournament.type === 'Professional' ? 'MI CLUB' : '', awayTeam: '', date: new Date().toISOString().split('T')[0], status: 'Scheduled', homeScore: 0, awayScore: 0, group: 'A', stage: 'Fase Regular', incidents: [] }); setEditingMatch(null); setShowMatchModal(true); }}
                      ...
                      {matches.map(m => (
                        <div key={m.id} className="bg-white dark:bg-[#0f1219] p-8 rounded-[3.5rem] border border-slate-200 dark:border-white/5 shadow-sm group hover:border-primary-600/30 transition-all flex flex-col md:flex-row items-center justify-between gap-8">
                            <div className="flex-1 text-center md:text-right px-4">
                              <h4 className={`text-2xl font-black uppercase tracking-tighter italic leading-none ${m.homeTeam === 'MI CLUB' ? 'text-primary-600' : 'text-slate-800 dark:text-white'}`}>{m.homeTeam}</h4>
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2 block">Local</span>
                            </div>
                            
                            <div className="flex flex-col items-center shrink-0 bg-slate-50 dark:bg-white/5 p-8 rounded-[3rem] border border-slate-100 dark:border-white/5 shadow-inner">
                              <div className="flex items-center gap-8 mb-4">
                                  <span className="text-5xl font-black italic dark:text-white">{m.status === 'Finished' ? m.homeScore : '-'}</span>
                                  <div className="w-12 h-12 bg-slate-950 text-white rounded-full flex items-center justify-center text-[10px] font-black italic shadow-lg">VS</div>
                                  <span className="text-5xl font-black italic dark:text-white">{m.status === 'Finished' ? m.awayScore : '-'}</span>
                              </div>
                              <div className="flex flex-col items-center gap-1">
                                  ...
                                  {m.group && <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-2">Grupo {m.group}</span>}
                              </div>
                            </div>

                            <div className="flex-1 text-center md:text-left px-4">
                              <h4 className={`text-2xl font-black uppercase tracking-tighter italic leading-none ${m.awayTeam === 'MI CLUB' ? 'text-primary-600' : 'text-slate-800 dark:text-white'}`}>{m.awayTeam}</h4>
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2 block">Visitante</span>
                            </div>

                            <div className="flex md:flex-col gap-2 shrink-0">
                              <button onClick={() => { setMatchForm(m); setEditingMatch(m); setShowMatchModal(true); }} className="p-4 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-primary-600 transition-all border border-transparent hover:border-primary-600/20"><Edit3 size={18} /></button>
                           ...
                           <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                              {standings.map((team, idx) => (
                                <tr key={team.name} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                   ...
                                   <td className={`px-8 py-6 font-black uppercase text-sm ${team.name === 'MI CLUB' ? 'text-primary-600' : 'text-slate-800 dark:text-white'}`}>{team.name}</td>
           ...
           <div className="bg-white dark:bg-[#0f121a] w-full max-w-4xl min-h-full md:min-h-0 md:rounded-[4rem] shadow-2xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col my-auto">
              <div className="p-8 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/40">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase italic tracking-tighter">Carga de Resultados e Incidencias</h3>
                </div>
              ...
              <div className="flex-1 p-8 md:p-12 space-y-12 overflow-y-auto custom-scrollbar">
                 {/* Cabecera de Equipos y Score */}
                 <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-20">
                    <div className="flex flex-col items-center gap-4 flex-1">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Equipo Local</label>
                       <input value={matchForm.homeTeam} onChange={e => setMatchForm({...matchForm, homeTeam: e.target.value.toUpperCase()})} placeholder="LOCAL" className="w-full bg-slate-50 dark:bg-slate-800 p-5 rounded-3xl font-black text-2xl text-center dark:text-white outline-none border-2 border-transparent focus:border-primary-600/30 shadow-inner" />
                       <input type="number" value={matchForm.homeScore} onChange={e => setMatchForm({...matchForm, homeScore: parseInt(e.target.value) || 0})} className="w-24 h-28 bg-slate-950 text-primary-600 rounded-[2rem] text-5xl font-black italic text-center outline-none border-4 border-primary-600/20 focus:border-primary-600 shadow-2xl" />
                    </div>
                    <div className="text-3xl font-black italic text-slate-200">VS</div>
                    <div className="flex flex-col items-center gap-4 flex-1">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Equipo Visitante</label>
                       <input value={matchForm.awayTeam} onChange={e => setMatchForm({...matchForm, awayTeam: e.target.value.toUpperCase()})} placeholder="VISITANTE" className="w-full bg-slate-50 dark:bg-slate-800 p-5 rounded-3xl font-black text-2xl text-center dark:text-white outline-none border-2 border-transparent focus:border-primary-600/30 shadow-inner" />
                       <input type="number" value={matchForm.awayScore} onChange={e => setMatchForm({...matchForm, awayScore: parseInt(e.target.value) || 0})} className="w-24 h-28 bg-slate-950 text-white rounded-[2rem] text-5xl font-black italic text-center outline-none border-4 border-white/5 focus:border-primary-600 shadow-2xl" />
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-4 gap-6 p-8 bg-slate-50 dark:bg-slate-800/40 rounded-[2.5rem] border border-slate-100 dark:border-white/5">
                    ...
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Fase / Estadio</label>
                 ...
                 {/* Sección de Incidencias (Jugadores del Club) */}
                 <div className="space-y-6">
                    <div className="flex justify-between items-center">
                       <h4 className="text-[11px] font-black uppercase text-slate-400 tracking-[0.2em] flex items-center gap-2">
                          <Activity size={18} className="text-primary-600" /> Registro de Incidencias por Jugador (MI CLUB)
                       </h4>
                       <div className="flex gap-2">
                          ...
                          <button onClick={() => addIncident('RedCard')} className="flex items-center gap-2 bg-red-500/10 text-red-600 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all"><AlertTriangle size={14}/> + Roja</button>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       {matchForm.incidents?.map((inc: any, idx: number) => (
                         <div key={inc.id} className="flex items-center gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-3xl border border-slate-100 dark:border-white/5 animate-fade-in group">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${inc.type === 'Goal' ? 'bg-emerald-500 text-white' : inc.type === 'YellowCard' ? 'bg-amber-500 text-white' : 'bg-red-500 text-white'}`}>
                            ...
                            <button 
                               onClick={() => {
                                 const newIncidents = matchForm.incidents.filter((_: any, i: number) => i !== idx);
                                 setMatchForm({...matchForm, incidents: newIncidents});
                               }}
              ...
              <div className="p-8 md:p-12 bg-slate-50 dark:bg-slate-800/40 border-t border-slate-100 dark:border-white/5 flex flex-col md:flex-row justify-end gap-4 shrink-0">
                 ...
                 <button onClick={handleSaveMatch} className="w-full md:w-auto bg-slate-900 dark:bg-primary-600 text-white px-16 py-5 rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-[1.02] active:scale-95 transition-all">Guardar Ficha Técnica</button>
              </div>
           </div>
        </div>
      )}
