...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside 
      className={`fixed left-0 top-0 h-screen bg-white dark:bg-[#0f1219] border-r border-slate-200 dark:border-white/5 flex flex-col z-[100] transition-all duration-500 ease-in-out 
        ${isCollapsed ? 'w-24 -left-20 md:left-0' : 'w-72 -left-72 md:left-0'} 
        ${!isCollapsed ? 'left-0 shadow-2xl' : ''} 
      `}
    >
      {/* Header with Toggle */}
      ...
      {/* Toggle Button - Visible en móvil para abrir/cerrar */}
      <button 
        ...
        className={`absolute -right-3 top-24 bg-primary-600 text-white p-2 rounded-full shadow-lg border-4 border-slate-50 dark:border-[#080a0f] hover:scale-110 transition-all z-[110] 
          ${isCollapsed ? 'translate-x-3 md:translate-x-0' : ''}
        `}
      >
      ...
      <nav className="flex-1 p-4 space-y-3 mt-4">
        {menu.map(item => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => { setView(item.id); if(window.innerWidth < 768) setCollapsed(true); }}
            ...
            >
              ...
              {isCollapsed && (
                <div className="absolute left-20 bg-slate-900 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-[100] border border-white/10 hidden md:block">
                  {item.label}
                </div>
              )}
            </button>
