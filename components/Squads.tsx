...
import { SportIcon } from './MasterData.tsx'; // Importamos el componente de íconos preciso
        ...
        <div className="flex gap-4 md:gap-8 overflow-x-auto pb-6 px-4 no-scrollbar items-center">
            {clubConfig.disciplines.map((disc) => {
                ...
                return (
                    ...
                    >
                        <div 
                            className={`w-20 h-20 md:w-32 md:h-32 rounded-full flex items-center justify-center transition-all duration-500 shadow-2xl relative border-4 ${isActive ? 'text-white shadow-primary-500/30 scale-100' : 'bg-white dark:bg-slate-900 text-slate-400 border-transparent'}`}
                            style={isActive ? { backgroundColor: clubConfig.primaryColor, borderColor: 'white' } : {}}
                        >
                            <SportIcon id={disc.icon || 'soccer'} size={isActive ? 48 : 32} />
                            {isActive && <div className="absolute inset-0 rounded-full animate-ping bg-white/20"></div>}
                        </div>
