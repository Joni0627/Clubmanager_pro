...
const MemberManagement: React.FC<MemberManagementProps> = ({ members, config, onSaveMember, onDeleteMember }) => {
  ...
  const tabs = [
    { id: 'identity', label: 'Identidad', icon: Fingerprint, color: 'text-primary-500' },
    { id: 'health', label: 'Salud', icon: Heart, color: 'text-rose-500' },
    { id: 'contacts', label: 'Contactos', icon: Contact2, color: 'text-emerald-500' },
    { id: 'sports', label: 'Deportes', icon: Briefcase, color: 'text-blue-500' },
    { id: 'system', label: 'Sistema', icon: ShieldCheck, color: 'text-violet-500' },
  ];

  return (
    <div className="p-8 max-w-7xl mx-auto animate-fade-in pb-40">
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button onClick={handleNew} className="bg-primary-600 text-white px-6 py-4 rounded-2xl shadow-xl shadow-primary-600/20 hover:scale-105 transition-all">
            <UserPlus size={20} />
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredMembers.map(member => (
          <div key={member.id} className="bg-white dark:bg-[#11141d] rounded-[2.5rem] p-8 border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
          ...
          <div className="bg-white dark:bg-[#0d1017] w-full max-w-6xl h-full lg:h-[85vh] lg:rounded-[3rem] shadow-2xl flex flex-col border border-slate-200 dark:border-white/10 overflow-hidden ring-1 ring-white/5">
            ...
            <div className="px-10 py-6 flex justify-between items-center border-b border-slate-100 dark:border-white/5 shrink-0 bg-slate-50/50 dark:bg-slate-900/40">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight italic">Legajo Maestro</h3>
                  <p className="text-[9px] font-black text-primary-600 uppercase tracking-[0.3em]">Plegma Sport • Gestión de Identidad</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="p-3 bg-white dark:bg-white/10 rounded-full hover:bg-red-500 hover:text-white transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-64 bg-slate-50/50 dark:bg-[#11141d] border-r border-slate-100 dark:border-white/5 flex flex-col shrink-0 overflow-y-auto no-scrollbar">
                ...
                <div className="p-8 flex flex-col items-center border-b border-slate-100 dark:border-white/5">
                  <div onClick={() => fileInputRef.current?.click()} className="w-36 h-36 rounded-[2.5rem] bg-slate-200 dark:bg-slate-900 border-2 border-primary-600/20 overflow-hidden shadow-lg relative group cursor-pointer mb-5 hover:scale-105 transition-all">
                ...
                <div className="max-w-3xl">
                    ...
                    <div className="space-y-8 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Documento</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Género</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Nacimiento</label>
                  ...
                  {activeTab === 'health' && (
                    <div className="space-y-8 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">G. Sanguíneo</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Peso (kg)</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Altura (cm)</label>
                  ...
                  {activeTab === 'contacts' && (
                    <div className="space-y-10 animate-fade-in">
                      ...
                      <section className="bg-emerald-500/5 p-8 rounded-[2rem] border border-emerald-500/10 space-y-6">
                        <h5 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-3">
                          <UserCheck size={16} /> Tutor / Contacto de Emergencia
                        </h5>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="space-y-2">
                            <label className="text-[8px] font-black text-emerald-600/60 uppercase tracking-widest ml-3">Documento Tutor</label>
                            <input value={formData.tutor?.dni || ''} onChange={e => setFormData({...formData, tutor: { ...(formData.tutor || { relationship: 'Padre', phone: '', name: '' }), dni: e.target.value }})} className="w-full p-3.5 bg-white dark:bg-[#0d1017] rounded-xl font-bold text-xs border border-emerald-500/10" placeholder="DNI" />
                          </div>
                          <div className="space-y-2">
                  ...
                  {activeTab === 'sports' && (
                    <div className="space-y-8 animate-fade-in">
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={addAssignment} className="flex items-center gap-2 text-primary-600 text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {formData.assignments?.map((as, idx) => {
                          ...
                          return (
                            <div key={as.id} className="bg-slate-50 dark:bg-[#151924] p-6 rounded-2xl border border-slate-100 dark:border-white/5 space-y-4 relative group shadow-sm">
                              <div className="flex justify-between items-center">
                                <select 
                                  value={as.role} 
                  ...
                  {activeTab === 'system' && (
                    <div className="space-y-10 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-3">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Tipo de Perfil</label>
                        ...
                        <div className="space-y-3">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Estado de Login</label>
                          <div className="flex gap-2 p-1 bg-slate-100 dark:bg-[#151924] rounded-xl border border-slate-200 dark:border-white/5">
                             <button onClick={() => setFormData({...formData, canLogin: true})} className={`flex-1 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${formData.canLogin ? 'bg-primary-600 text-white shadow-lg' : 'text-slate-400'}`}>Habilitar</button>
                             <button onClick={() => setFormData({...formData, canLogin: false})} className={`flex-1 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${!formData.canLogin ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400'}`}>Bloquear</button>
                          </div>
                        </div>
                        {formData.canLogin && (
                          <div className="space-y-3 col-span-1 md:col-span-2">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">Nombre de Usuario</label>
                      ...
                      <div className="p-8 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 flex items-center gap-5">
                         <BadgeCheck className="text-emerald-500 shrink-0" size={32} />
                         <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest leading-relaxed">Este miembro se encuentra habilitado bajo las normativas vigentes del club.</p>
                      </div>
                    </div>
                  )}
            ...
            <div className="px-10 py-6 border-t border-slate-100 dark:border-white/5 flex justify-end bg-slate-50/50 dark:bg-slate-900/40 shrink-0">
              <button 
                onClick={handleSave} 
                disabled={isSaving}
                className="flex items-center gap-4 bg-primary-600 text-white px-12 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-xl shadow-primary-600/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
              >
