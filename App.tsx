...
// Fixed: Added 'Users' to the lucide-react icons import list
...
function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        // Intentar cargar Configuración del Club
        const { data: configData, error: configError } = await db.clubConfig.get();
        if (!configError && configData) {
          setClubConfig(configData);
        } else {
          console.warn("No se encontró configuración en Supabase, usando valores por defecto.");
        }
        ...
        if (!playersError && playersData) {
          ...
          console.warn("No se pudieron cargar los jugadores.");
        }
  ...
  const updateClubConfig = async (newConfig: ClubConfig) => {
      ...
      try {
        ...
        console.error("Error persistiendo configuración:", e);
      }
  };
  ...
  const renderContent = () => {
    if (isLoading) return (
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900">
            ...
            <p className="text-slate-500 font-medium animate-pulse">Conectando con PlegmaSport...</p>
        </div>
    );
    ...
    switch (currentView) {
      case 'dashboard':
        return <Dashboard />;
      case 'medical':
        return <MedicalDashboard players={players} />;
      case 'fees':
          ...
          <div className="p-6 h-full flex flex-col">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
               <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Datos sincronizados con Supabase</p>
               </div>
               ...
               <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
                   <select 
                        ...
                        className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none dark:text-white"
                   >
            ...
            <div className="overflow-y-auto space-y-8 pb-10">
                {Object.keys(groupedPlayers).length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-slate-400 bg-white dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
                        ...
                        <p className="font-medium">No hay jugadores registrados en la base de datos.</p>
                                            ...
                                            viewMode === 'grid' ? (
                                                <div key={player.id} onClick={() => setSelectedPlayer(player)} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden cursor-pointer hover:shadow-lg transition-all group">
                                                    <div className="h-40 overflow-hidden relative">
                                                        <img src={player.photoUrl || 'https://via.placeholder.com/400'} alt={player.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                                                ...
                                                <div key={player.id} onClick={() => setSelectedPlayer(player)} className="p-4 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer">
                                                    <div className="flex items-center gap-3">
                                                        <img src={player.photoUrl || 'https://via.placeholder.com/400'} className="w-10 h-10 rounded-full object-cover" />
                                                        <div>
      ...
      case 'master-data':
        return <MasterData clubConfig={clubConfig} setClubConfig={updateClubConfig} />;
      case 'attendance':
        return <AttendanceTracker players={players} />;
      default:
        return <Dashboard />;
    }
  };
      ...
      <main className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'ml-20' : 'ml-64'}`}>
        ...
        <div className="py-4 text-center border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 mt-auto">
          <p className="text-xs text-slate-400 font-medium">
            &copy; {new Date().getFullYear()} {clubConfig.name} <span className="text-primary-500">PlegmaSport</span>.
          </p>
        </div>
      </main>
